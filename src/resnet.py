...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, group_norm=0, weight_stand=0):
        ...
        if weight_stand:
            self.conv1 = Conv2d_stand(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
            self.conv2 = Conv2d_stand(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion * planes:
            if weight_stand:
                self.shortcut = nn.Sequential(
                    Conv2d_stand(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False),
...
class ResNet_cifar(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, group_norm=8, weight_stand=0):
        super(ResNet_cifar, self).__init__()
        self.in_planes = 16

        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        if weight_stand:
            self.conv1 = Conv2d_stand(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = norm2d(16, group_norm)
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1, group_norm=8, weight_stand=weight_stand)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2, group_norm=16, weight_stand=weight_stand)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2, group_norm=32, weight_stand=weight_stand)
        self.fc = nn.Linear(64 * block.expansion, num_classes)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))

    def _make_layer(self, block, planes, num_blocks, stride, group_norm=0, weight_stand=0):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        for stride in strides:
...
# ImageNet models
def ResNet18(num_classes=10, weight_stand=0):
    return ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, weight_stand=weight_stand)
...
def ResNet32_test(num_classes=10, weight_stand=0):
    depth = 32
    n = (depth - 2) // 6
    return ResNet_cifar(BasicBlock, [n, n, n], weight_stand=weight_stand, num_classes=num_classes)
